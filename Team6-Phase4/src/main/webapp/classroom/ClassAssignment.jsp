...
<%
	...
	out.println("<h2>숙제 게시판</h2>");
	
	String query_assignment = "SELECT AssignmentNumber FROM COURSE_ASSIGNMENT WHERE CID = '" + user_class + "' ORDER BY AssignmentNumber DESC";
	...
	// href로 수정 예정
	while (rs.next()) {
		String title = "숙제 " + (rs.getInt(1));
%>
		<article><a id="link" href = "assignment.jsp?no=<%=rs.getInt(1)%>"><%out.println(title);%></a></article>
<%
	}	
	out.println("</div>");
	...
	if (user_status.equals("teacher")) {
		...
		out.println("<button type=\"button\" onclick=\"location.href='write_assignment.jsp' \">숙제 작성</button>");
		out.println("</div>");
		out.println("</div><br>");
	}
	out.println("</div>");
