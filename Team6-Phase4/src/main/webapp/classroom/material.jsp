...
<% 
	...
	out.println("<textarea id = \"name_area\" style = \"overflow:hidden\" readonly = \"readonly\" disabled>자료 " + (Integer.parseInt(m_num)) + "</textarea><br>");
	out.println("<textarea id = \"write_section\" style = \"overflow:hidden\" readonly=\"readonly\" disabled>");
	
	String query_material = "SELECT Material FROM COURSE_MATERIAL WHERE CID = '" + user_class + "' AND MaterialNumber = " + m_num;
...
%>	
	...
	<button type = "button" onclick="location.href='update_material.jsp?no=<%=m_num%>'">수정하기</button>
	<button type = "button" onclick="location.href='delete_material.jsp?no=<%=m_num%>'">삭제하기</button>
	</div>
<%	
	}
	out.println("</div><br>");
