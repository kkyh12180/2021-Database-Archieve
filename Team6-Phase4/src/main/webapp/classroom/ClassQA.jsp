...
<%
	...
	out.println("<h2>질문과 답변 게시판</h2>");
	
	String query_qa = "SELECT QuestionNumber FROM QA WHERE CID = '" + user_class + "' ORDER BY QuestionNumber DESC";
	...
	// href로 수정 예정
	while (rs.next()) {
		String title = "질문 " + (rs.getInt(1));
%>
		<article><a id="link" href = "question.jsp?no=<%=rs.getInt(1)%>"><%out.println(title);%></a></article>
<%
	}	
	out.println("</div>");
	...
	if (user_status.equals("student")) {
		...
		out.println("<button type=\"button\" onclick=\"location.href='write_question.jsp' \">질문하기</button>");
		out.println("</div>");
		out.println("</div><br>");
	}
	out.println("</div>");
